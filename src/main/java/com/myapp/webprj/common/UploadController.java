...
public class UploadController {
    ...
    //업로드 파일 저장 경로
    private static final String uploadPath = "D:\\developing_lyj\\upload";

    //업로드 form jsp 파일을 포워딩하는 화면처리
    ...
    public String upload(@RequestParam("file") List<MultipartFile> fileList) {
        ...
        /*//업로드파일 저장 경로
        //윈도우는 \\ 리눅스는 /
        String uploadPath = "D:\\developing_lyj\\upload";
*/
        for (MultipartFile file : fileList) {
            ...
            //세이브파일 객체 생성
            //생성자의 첫번째 파라미터로 저장 경로를, 두번째 파라미터로 파일명을 넣어주세요
    ...
    //파일 로딩 요청 처리
    //요청 URI : /loadFile?fileName=/2021/04/22/dajldkjgre_cat.jpg
    @GetMapping("/loadFile")
    @ResponseBody
    public ResponseEntity<byte[]> loadFile(String fileName) {
        ...
        //클라이언트가 요청한 파일명을 이용하여 파일의 풀 경로를 만들고 파일 객체를 생성
        ...
        //서버에 해당 파일이 저장되어 있었다면 InputStream 객체를 통해 파일을 불러옴
        try {
