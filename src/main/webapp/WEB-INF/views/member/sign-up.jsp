    ...
    $(document).ready(function () {
        ...
        $idInput.on('keyup', e => {
            ...
            if ($idInput.val().trim() === '') {
                ...
                //test 메서드는 정규표현식을 검증하여 값이 정규표현식과 일치하면 true
            //일치하지 않으면 false를 리턴
        ...
        $('#password').on('keyup', function () {
            //비밀번호 공백 확인
            if ($("#password").val() === "") { //패스워드 미입력 확인
                $('#password').css("background-color", "pink");
                $('#pwChk').html('<b style="font-size:14px;color:red;">[패스워드는 필수정보!]</b>');
                chk2 = false;
            }
            //비밀번호 유효성검사
            else if (!getPwCheck.test($("#password").val()) || $("#password").val().length < 8) { //특수문자 포함 정규표현식 혹은 길이 검증
                $('#password').css("background-color", "pink");
        ...
        $('#password_check').on('keyup', function () {
            ...
            //비밀번호 확인란 유효성검사
            else if ($("#password").val() != $("#password_check").val()) { //패스워드란과 확인란의 내용 비교
                $('#password_check').css("background-color", "pink");
        ...
        $('#user_email').on('keyup', function () {
            ...
            //이메일값 유효성검사
                ...
                //이메일 중복확인 비동기 통신
                fetch('/check?kind=email&info=' + $("#user_email").val())
                    .then(res => res.text())
                    .then(flag => {
                        // console.log(flag);
                        if (flag === 'true') {
                            ...
                            $('#user_email').css('background', 'aqua');
